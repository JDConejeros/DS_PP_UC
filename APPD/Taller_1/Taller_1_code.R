...
# Tutorial Inicial: https://jdconejeros.shinyapps.io/Guia_inicial_R_2022/
########################################################################/
...
# La mejor opción: Generar proyectos en R (revisar video clase 1)
# Limpiar memoria
rm(resultado)
rm(list=ls())
...
cualquier_cosa = c(1, "Hola", "Chaito", 4)
cualquier_cosa/2

as.numeric(cualquier_cosa[1])/2

cualquier_cosa2 <- as.numeric(c(cualquier_cosa[1], cualquier_cosa[4]))
cualquier_cosa2/2
...
# Missing en las matrices
x[1,1] <- NA

is.na(x[1,1]) <- 1

x[1,1] <- ifelse(is.na(x[1,1]), 2, x[1,1])
x
...
data <- read.csv("APPD/Taller_1/data/simce2m2016_extracto.csv")
...
write.table(data, file="APPD/Taller_1/data/simce_modificada.txt", sep="\t")

# 7.4.2 Base en formato .csv  ----------------------------------------------------------------
write.csv(data, file="APPD/Taller_1/data/simce_modificada.csv", row.names = FALSE)

# 7.4.3 Base en formato .xlsx  ----------------------------------------------------------------
library(openxlsx)
write.xlsx(data, file="APPD/Taller_1/data/simce_modificada.xlsx")

# 7.4.4 Base en formato .sav  ----------------------------------------------------------------
library(haven)
write_sav(data, "APPD/Taller_1/data/simce_modificada.sav")

# 7.4.5 Base en formato .sas  ----------------------------------------------------------------
write_sas(data, "APPD/Taller_1/data/simce_modificada.sas7bdat")

# 7.4.5 Base en formato .dta  ----------------------------------------------------------------
write_dta(data, "APPD/Taller_1/data/simce_modificada.dta")

# 7.4.6 Base en formato .Rdata  ----------------------------------------------------------------
save(data, file="APPD/Taller_1/data/simce_modificada.Rdata")
